...
namespace DomainModel.Classes.Products.ConcreteProducts
{
    public class ELA(IColor ic, IColor ec) : WoodAbstractProduct(ic, ec)
    {
        ...
        {
            return GetFullDoubleDimensionPrice(985M, area_sqmm, m.OpaqueGlass, m.WireCover, m.Height_mm, m.Width_mm, m.FrameType);
        }
        public override decimal GetPrice_DoubleDimFixed(DoubleDimFixedMaterial m, long area_sqmm)
        {
            return GetFullDoubleDimensionPrice(568M, area_sqmm, m.OpaqueGlass, m.WireCover, m.Height_mm, m.Width_mm, m.FrameType);
        }
    }
}
